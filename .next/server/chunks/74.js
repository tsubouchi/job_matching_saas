...
const { initializeApp  } = __webpack_require__(4324);
const { getDatabase , ref , set , get , push , update , remove  } = __webpack_require__(6666);
// Firebase設定
// 環境変数から設定を取得
 ...
 */ async function getChatHistory(chatId) {
    try {
        ...
        return Object.entries(messages).map(([id, data])=>({
                id,
                ...data
 ...
 */ async function getUserChats(userId) {
    try {
        ...
        return Object.entries(chats).filter(([_, data])=>data.participants && data.participants[userId]).map(([id, data])=>({
                id,
                ...data
